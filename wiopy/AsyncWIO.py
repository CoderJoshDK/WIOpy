...
class AsyncWalmartIO:
    ...
    async def post_browsed_products(self, itemId: str) -> List[WalmartProduct]:
        ...
        For instance, if you know that one of your customers has recently viewed an Xbox one, it is likely that they may be also interested in purchasing a PlayStation 4, or Nintendo Wii.
    ...
    async def bulk_product_lookup(
        self, ids: Union[str, List[str]], amount: int = 20, retries: int = 1, **kwargs
    ):
        """
    ...
    async def product_recommendation(self, itemId: str) -> List[WalmartProduct]:
         ...
         * Re-targeting on an e-commerce storefront anchored on a shared UPC between multiple merchants
         * An up-sell to an existing customer presenting an array of products due to knowing their order history
            ...
            async with session.get(
                ...
                if status_code == 400:
                    ...
                    raise InvalidRequestException(
                        status_code, detail=jsonData["errors"][0]["message"]
                    )
